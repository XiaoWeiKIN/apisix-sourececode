...
local function init(env)
    ...
    -- 执行openresty -V 2>&1 | grep http_stub_status_module命令，
    -- 主要是校验编译openresty在编译的时候是否编译了http_stub_status_module模块
    ...
    elseif type(yaml_conf.apisix.node_listen) == "table" then
        ...
        for index, value in ipairs(yaml_conf.apisix.node_listen) do
            if type(value) == "number" then
                table_insert(node_listen, index, {port = value}) -- index 不计算下标
    ...
    if util.is_32bit_arch() then
        --worker_rlimit_core: nginx的配置，限制coredump核心转储文件的大小，用于在不重启主进程的情况下增加限制。
        sys_conf["worker_rlimit_core"] = "4G"
    else
        sys_conf["worker_rlimit_core"] = "16G"
    end
    -- 将apisix和nginx配置转到sys_conf
...
local function start(env, ...)
    ...
    -- check running 检查APISIX进程是否在运行中
